...
function Moviedetails() {
  ...
  return info ? (
    <div
      ...
      className=' relative w-screen h-[175vh] px-[10%] py-10 text-white'
    >
      {/* part-1 navigation */}
      ...
      <div className='w-full flex gap-10 items-start mt-6'>
        ...
        <div className='bg-white/10 p-6 rounded text-white w-full'>
          ...
          <small 
          className='text-2xl font-bold text-zinc-300'>({info.detail.release_date.split("-")[0]})
           </small>
          </h1>
          
       <div className='mt-3 mb-10 flex text-zinc-100 items-center gap-x-5 '>
         <span 
      className='bg-yellow-600 rounded-full text-xl font-semibold  text-white w-[5vh] h-[5vh] flex justify-center items-center'>
     ...
     <h1>{info.detail.runtime}min</h1>
        ...
        <Link  to={`${pathname}/trailer`}  className='p-5 bg-[#6556CD] rounded-lg ' ><i className='ri-play-fill mr-2'></i>Play Trailer</Link>
      ...
      <div className='bg-gradient-to-r from-black/70 via-black/30 to-transparent mt-2 p-6 rounded text-white w-full'>
        ...
        {info.watchproviders?.flatrate?.length > 0 ? (
          ...
          <p className='text-zinc-300'>No streaming providers available in India.</p>
        )}
      </div>
     ...
     <div className='bg-gradient-to-r from-black/70 via-black/30 to-transparent mt-2 p-6 rounded text-white w-full'>
      ...
      <div className='bg-gradient-to-r from-black/70 via-black/30 to-transparent mt-2 p-6 rounded text-white w-full'>
        ...
        {info.watchproviders?.buy?.length > 0 ? (
          ...
          <p className='text-zinc-300'>No purchase options available in India.</p>
        )}
      </div>


    {/* part 4 Reccomendations and similar stuff */}
    <hr className='mt-10 mb-5 border-none h-[1px] bg-zinc-300'/>
    <h1 className='mt-10 text-2xl font-semibold text-white'>Recommendations & Similar Items</h1>
    <HorizontalCards 
     data= {
      info.recommendations.lenght >0  ? 
